...
class Context3D 
{
   ...
   public function drawTriangles(indexBuffer:IndexBuffer3D, firstIndex:Int = 0, numTriangles:Int = -1):Void 
   {
      ...
      var firstIndexByteOffset = firstIndex * 2; //Using GL.UNSIGNED_Short

      GL.bindBuffer(GL.ELEMENT_ARRAY_BUFFER, indexBuffer.glBuffer);
      GL.drawElements(GL.TRIANGLES, numIndices, GL.UNSIGNED_SHORT, firstIndexByteOffset);
   }
  ...
  var stencilCompareMode:Int;
  var stencilRef:Int;
  var stencilWriteMask:Int;
  var stencilReadMask:Int;
   public function setStencilActions(?triangleFace:Int, ?compareMode:Int, ?actionOnBothPass:Context3DStencilAction, ?actionOnDepthFail:Context3DStencilAction, ?actionOnDepthPassStencilFail:Context3DStencilAction):Void 
   {
      // TODO
     function m(x:Context3DStencilAction) {
       return switch (x) {
       case KEEP: GL.KEEP;
       case INVERT: GL.INVERT;
       case SET: GL.REPLACE;
       case _: throw "Not supported " + x;
       }
     }

     this.stencilCompareMode = compareMode;

     GL.stencilOp(m(actionOnBothPass), m(actionOnDepthFail), m(actionOnDepthPassStencilFail));
